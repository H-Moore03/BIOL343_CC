...
1. **Henry Moore** - 20474140
    + [BIOL343_CC](https://github.com/H-Moore03/BIOL343_CC)
1.**Ziadie Diebel** - 20463388
    + [BIOL343_CC](https://github.com/ziadiediebel-maker/BIOL343_CodingChallenges)
2.**Nathan Rhodes** - 20560116
    + [BIOL343_CC](https://github.com/28Nate28/BIOL343_CC)
3.**Anna Goldfarb** - 20455917
    + [BIOL343_CC](https://github.com/Anna-Goldfarb/BIOL343_CC)
4.**Ellie Maki** - 20419523
    + [BIOL343_CC](https://github.com/EllieMaki/BIOL343)
...
---------|-------------|---------
population_type |Describes what population the plant is from between sexual and asexual populations | No units
ovary_width |Describes the width of the ovary | mm
ovary_height |Describes the height of the ovary | mm
flower_number |Describes the number of flowers on each plant | No units (just a count)
...
```{r import-clean}
cc1_data <- read.csv("A1_Data_Excel.csv")

# Cleaning Data

cc1_data_mod <- cc1_data[1:46, c(1,5,6,8)]
str(cc1_data_mod)

# Adding ovary area column
...
$$OvaryHeight\times Ovary Width=Ovary Area$$
Ovary Height uses $mm$, Ovary Width uses $mm$, and Ovary Area uses $mm^2$
...
kable(area_means, caption="Summary Table of Ovary Area means in each population")
```
...
plot1<-ggplot(aes(x=ovary_area), data=cc1_data_mod) + geom_histogram()+stat_bin(bins=15) + facet_wrap(vars(population_type))+xlab("Ovary Area (mm^2)")+ylab("Count")+ggtitle("Ovary Area in Two Different Populations")
...
![*Solanum pimpinellifolium*](./Images/study_species.jpg)

Source: [Click Here](https://onq.queensu.ca/d2l/lms/dropbox/user/folder_submit_files.d2l?db=492096&grpid=1208698&isprv=0&bp=0&ou=1208667)
